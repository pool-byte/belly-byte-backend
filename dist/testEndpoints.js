...
async function runTests() {
    ...
    try {
        ...
        const bun = await Item_1.default.create({ name: 'Bun', category: 'Raw Material', unit: 'pcs', minStockAlert: 20, startingQuantity: 100, currentQuantity: 100 });
        const sauce = await Item_1.default.create({ name: 'Sauce', category: 'Raw Material', unit: 'kg', minStockAlert: 1.0, startingQuantity: 5.0, currentQuantity: 5.0 }); // 5kg
        const mozzarella = await Item_1.default.create({ name: 'Mozzarella', category: 'Raw Material', unit: 'kg', minStockAlert: 1.0, startingQuantity: 4.0, currentQuantity: 4.0 }); // 4kg
        const vegMix = await Item_1.default.create({ name: 'Corn/Onion/Capsicum', category: 'Raw Material', unit: 'kg', minStockAlert: 2.0, startingQuantity: 10.0, currentQuantity: 10.0 }); // 10kg
        const coffeePowder = await Item_1.default.create({ name: 'Coffee Powder', category: 'Raw Material', unit: 'kg', minStockAlert: 0.2, startingQuantity: 1.0, currentQuantity: 1.0 }); // 1kg = 1000g
        const sugar = await Item_1.default.create({ name: 'Sugar', category: 'Raw Material', unit: 'kg', minStockAlert: 0.5, startingQuantity: 2.0, currentQuantity: 2.0 }); // 2kg
        const milk = await Item_1.default.create({ name: 'Milk', category: 'L', minStockAlert: 3.0, startingQuantity: 10.0, currentQuantity: 10.0 }); // 10L = 10000ml
        const ice = await Item_1.default.create({ name: 'Ice', category: 'kg', unit: 'kg', minStockAlert: 2.0, startingQuantity: 10.0, currentQuantity: 10.0 });
        const vanilla = await Item_1.default.create({ name: 'Vanilla Flavor', category: 'Consumable', unit: 'ml', minStockAlert: 50, startingQuantity: 500, currentQuantity: 500 }); // 500ml
        ...
        const sandwichRecipe = await FoodItem_1.default.create({
            name: 'Sandwich',
            price: 80,
            ingredients: [
        ...
        const coffeeRecipe = await FoodItem_1.default.create({
            name: '350ml Coffee',
            price: 60,
            ingredients: [
        ...
        const test35gInKg = (0, unitConverter_1.convertQuantity)(35, 'g', 'kg');
        const test165mlInL = (0, unitConverter_1.convertQuantity)(165, 'ml', 'L');
        ...
        const sale1 = await Sale_1.default.create({
            shiftId: shift._id,
        ...
        const sale2 = await Sale_1.default.create({
            shiftId: shift._id,
        ...
        const coffeePowderItem = await Item_1.default.findById(coffeePowder._id);
        if (coffeePowderItem) {
            const usedInKg = (0, unitConverter_1.convertQuantity)(10 * 30, 'g', 'kg'); // 300g -> 0.3kg
        ...
        const coffeePowderAfter = await Item_1.default.findById(coffeePowder._id);
